<x-layouts.app title="Data Poli">
    ...
    <div class="p-4 sm:p-6">
        <div class="mx-auto max-w-7xl">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Data Poli</h1>
                    <p class="mt-1 text-sm text-slate-600">
                        Kelola daftar poliklinik dan deskripsi layanan.
                    </p>
                </div>

                <div class="flex flex-wrap items-center gap-2">
                    <button type="button" id="exportPoli"
            ...
            <div class="mt-6 rounded-3xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div class="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
                        <div class="relative w-full sm:max-w-sm">
                            ...
                            <input id="searchPoli" type="text"
                                ...
                                placeholder="Cari nama poli atau keterangan…">
                        </div>
                        ...
                        <div class="hidden sm:flex items-center gap-2 text-xs text-slate-500">
                            ...
                            <span class="inline-flex items-center rounded-full px-2 py-1 badge-brand">
                                Tampil: <span id="visiblePoli" class="ml-1 font-semibold text-slate-900">{{ $total }}</span>
                            </span>
                        </div>
                    </div>

                    <div class="flex items-center justify-between sm:hidden">
                        <span class="text-xs text-slate-500">Tampil: <span id="visiblePoliSm" class="font-semibold text-slate-900">{{ $total }}</span></span>
                        <span class="text-xs text-slate-500">Total: <span class="font-semibold text-slate-900">{{ $total }}</span></span>
                    </div>
                </div>

                <div class="overflow-x-auto">
                    <table id="tablePoli" class="min-w-full text-sm">
                        <thead class="sticky top-0 bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-600">
                            <tr>
                                <th class="whitespace-nowrap px-4 py-3">#</th>
                                <th class="whitespace-nowrap px-4 py-3">Nama Poli</th>
                                <th class="whitespace-nowrap px-4 py-3">Keterangan</th>
                        ...
                        <tbody class="divide-y divide-slate-200 bg-white">
                            @forelse ($polis as $index => $poli)
                                @php
                                    $search = implode(' ', [$poli->nama_poli, $poli->keterangan]);
                                @endphp
                                <tr data-row="1" data-search="{{ $search }}" class="hover:bg-slate-50/70">
                                    <td class="px-4 py-3 text-slate-600">{{ $index + 1 }}</td>
                                    <td class="px-4 py-3">
                                        <div class="font-semibold text-slate-900">{{ $poli->nama_poli }}</div>
                                        <div class="mt-0.5 text-xs text-slate-500">Master Poli</div>
                                    </td>
                                    <td class="px-4 py-3 text-slate-700">
                                        {{ \Illuminate\Support\Str::limit($poli->keterangan, 80) }}
                                    </td>
                                ...
                                <tr>
                                    <td colspan="4" class="px-4 py-10 text-center">
                                        <div class="mx-auto max-w-md">
                                            ...
                                            <div class="mt-4 text-base font-semibold text-slate-900">Belum ada poli</div>
                                            <div class="mt-1 text-sm text-slate-600">Buat poliklinik untuk kebutuhan jadwal dokter dan pendaftaran pasien.</div>
                                            <div class="mt-5">
                                                <a href="{{ route('admin.poli.create') }}"
                                                    class="inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold btn-brand shadow-sm ring-brand">
                                                    <i class="fa-solid fa-plus text-xs"></i>
                ...
                <div id="emptyPoliSearch" class="hidden border-t border-slate-200 p-6">
                    <div class="flex flex-col items-center justify-center gap-2 text-center">
                        ...
                        <button type="button" id="resetPoli"
    ...
    <script>
        (function() {
            ...
            function init() {
                const search = document.getElementById('searchPoli');
                const table = document.getElementById('tablePoli');
                const empty = document.getElementById('emptyPoliSearch');
                const resetBtn = document.getElementById('resetPoli');
                const exportBtn = document.getElementById('exportPoli');
                const visible = document.getElementById('visiblePoli');
                const visibleSm = document.getElementById('visiblePoliSm');
                ...
                exportBtn?.addEventListener('click', () => exportTableToCSV(table, 'data-poli.csv'));
