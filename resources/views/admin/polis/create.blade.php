    ...
    <div class="p-4 sm:p-6">
        <div class="mx-auto max-w-3xl">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Tambah Poli</h1>
                    <p class="mt-1 text-sm text-slate-600">
                        Buat poli baru untuk pengelompokan layanan dan penugasan dokter.
                    </p>
                </div>
            ...
            <div class="mt-6 rounded-3xl border border-slate-200 bg-white shadow-sm">
                <form action="{{ route('admin.poli.store') }}" method="POST" id="poliForm">
                    ...
                    <div class="border-b border-slate-200 p-5">
                        ...
                        <p class="mt-1 text-sm text-slate-600">Nama poli wajib diisi, keterangan bersifat opsional.</p>
                    </div>
                    ...
                    <div class="grid gap-5 p-5">
                        <div>
                            ...
                            <input id="nama_poli" name="nama_poli" type="text" required
                                ...
                                class="focus-brand mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 @error('nama_poli') is-invalid @enderror">
                            @error('nama_poli')
                        ...
                        <div>
                            ...
                            <textarea id="keterangan" name="keterangan" rows="3"
                                placeholder="Opsional — misalnya jam layanan atau catatan poli"
                                class="focus-brand mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 @error('keterangan') is-invalid @enderror">{{ old('keterangan') }}</textarea>
                            @error('keterangan')
                                <div class="mt-2 text-sm font-semibold text-rose-600">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="flex flex-col-reverse gap-3 border-t border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <button type="button"
                                ...
                                onclick="confirmSubmit('#poliForm', 'Simpan poli baru?', 'Data poli akan ditambahkan ke daftar poli.')">
