    ...
    <div class="p-4 sm:p-6">
        <div class="mx-auto max-w-4xl">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Edit Pasien</h1>
                    <p class="mt-1 text-sm text-slate-600">
                        Perbarui data pasien. Password boleh dikosongkan jika tidak ingin diubah.
                    </p>
                </div>
                ...
                <form action="{{ route('admin.pasien.update', $pasien->id) }}" method="POST" id="pasienEditForm">
                    ...
                    <div class="border-b border-slate-200 p-5">
                        <div class="flex items-center justify-between">
                            <div class="text-sm font-semibold text-slate-900">Data Pasien</div>
                            <span class="rounded-full px-2 py-1 text-xs font-semibold badge-brand">Edit</span>
                        </div>
                        <p class="mt-1 text-sm text-slate-600">Pastikan kontak pasien dapat dihubungi.</p>
                    </div>

                    <div class="grid gap-5 p-5 sm:grid-cols-2">
                        <div>
                            <label for="nama" class="text-sm font-semibold text-slate-700">Nama Pasien <span class="text-rose-600">*</span></label>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea id="alamat" name="alamat" rows="3" required
                                class="focus-brand mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 @error('alamat') is-invalid @enderror">{{ old('alamat', $pasien->alamat) }}</textarea>
                            @error('alamat')
                                <div class="mt-2 text-sm font-semibold text-rose-600">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="border-t border-slate-200 p-5">
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-slate-600">Opsional. Aktifkan jika ingin mengganti password pasien.</p>
                            </div>
                        ...
                        <div id="pwPanel" class="mt-4 hidden">
                            ...
                            <div class="mt-2 text-xs help-muted">Minimal 6 karakter.</div>
                            @error('password')
                                <div class="mt-2 text-sm font-semibold text-rose-600">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="flex flex-col-reverse gap-3 border-t border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <button type="button"
                                ...
                                onclick="confirmSubmit('#pasienEditForm', 'Update data pasien?', 'Perubahan akan disimpan ke sistem.')">
